...
@section('content')
    ...
    <div class="container-fluid mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="relative inline-block" id="createBtnWrapper">
                ...
                <div id="productTypePickerMenu"
                    class="hidden absolute right-0 z-50 mt-2 w-56 bg-white rounded-lg shadow-xl border border-slate-200 overflow-hidden">
                        ...
                        @foreach($productTypes as $type)
                            <li>
                                <button type="button"
                                    ...
                                    data-type-id="{{ $type->idtbl_product_types }}" data-type-name="{{ $type->name }}">
                                    <span
                                        class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-blue-100 text-blue-700 text-[11px] font-bold">{{ strtoupper(substr($type->skuname ?? $type->name, 0, 2)) }}</span>
                                    <span>{{ $type->name }}</span>
                                </button>
                            </li>
                        @endforeach
    ...
    <div id="createGemstoneModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 sm:p-6"
        ...
        <div class="bg-white rounded-md shadow-2xl w-full max-w-5xl flex flex-col h-[95vh] relative overflow-hidden">
            ...
            <div class="px-6 py-4 flex-shrink-0 bg-[#f8fafc] rounded-t-md">
                <div class="flex justify-between items-start">
                    <button type="button" id="closeGemstoneModalBtn"
                    ...
                    <a href="{{ route('inventory.myinventory.new') }}"
                        class="text-slate-500 hover:text-slate-800 transition-colors bg-white p-2.5 rounded-md border border-slate-200 shadow-sm inline-flex items-center justify-center">
            ...
            <div class="px-6 border-b border-slate-200 flex-shrink-0 bg-white flex w-full">
                <button type="button" class="tab-btn active py-3.5 font-semibold text-[14px] flex-1 text-center"
                    data-target="#tab-quick-view">Quick View</button>
                <button type="button" class="tab-btn py-3.5 font-semibold text-[14px] flex-1 text-center text-slate-500"
                    data-target="#tab-pricing">Pricing</button>
                <button type="button" class="tab-btn py-3.5 font-semibold text-[14px] flex-1 text-center text-slate-500"
                    data-target="#tab-history">History</button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar bg-white relative">
                <form id="createGemstoneForm" action="{{ route('inventory.myinventory.store') }}" method="POST">
                    ...
                    <div id="tab-quick-view" class="tab-content active block px-6 py-6 pb-20">
                        ...
                        {{-- Upload Box --}}
                        <div
                            class="w-[120px] h-[120px] bg-[#f1f5f9] rounded-md flex flex-col items-center justify-center cursor-pointer hover:bg-slate-200 transition-colors mb-6 border border-transparent">
                        ...
                        {{-- Section: Inventory Type --}}
                        <div class="section-header !mt-0">Inventory Type</div>
                        <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <label
                                class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none border-blue-500 ring-1 ring-blue-500 inventory-type-label">
                                <input type="radio" name="inventory_type" value="individual"
                                    class="sr-only inventory-type-radio" checked>
                                <span class="flex flex-1">
                                    <span class="flex flex-col">
                                        <span class="block text-sm font-medium text-slate-900 flex items-center gap-2"><i
                                                class="fa-regular fa-gem text-blue-500"></i> Individual (Single
                                            piece)</span>
                                ...
                                <svg class="h-5 w-5 text-blue-600 check-icon" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <label
                                class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none border-slate-300 hover:border-slate-400 inventory-type-label">
                                <input type="radio" name="inventory_type" value="lot" class="sr-only inventory-type-radio">
                                <span class="flex flex-1">
                                    <span class="flex flex-col">
                                        <span class="block text-sm font-medium text-slate-900 flex items-center gap-2"><i
                                                class="fa-solid fa-boxes-stacked text-slate-400"></i> Lot (Multiple
                                            pieces)</span>
                                        <span class="mt-1 flex items-center text-xs text-slate-500">
                                            <ul class="list-disc pl-4 mt-1 space-y-1">
                                ...
                                <svg class="h-5 w-5 text-blue-600 check-icon hidden" viewBox="0 0 20 20"
                                    fill="currentColor">
                                    <path fill-rule="evenodd"
                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z"
                                        clip-rule="evenodd" />
                        ...
                        <div class="section-header">Product attributes</div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5">
                                ...
                                <div class="flex gap-2">
                                    <div class="w-1/2">
                                        ...
                                        <div class="relative searchable-dropdown" id="ddSkuPrefixWrapper">
                                            ...
                                            <button type="button" id="ddSkuPrefixBtn"
                                    ...
                                    <div class="w-1/2">
                                        ...
                                        <input type="text" name="sku_number" id="skuNumberInput"
                                            value="{{ old('sku_number', '') }}" class="form-control w-full px-3">
                                    </div>
                                </div>
                            </div>
                            ...
                            {{-- Variety --}}
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5">Variety <span
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddVarietyWrapper">
                                    ...
                                    <button type="button" id="ddVarietyBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddVarietyPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <div class="p-2 border-b border-slate-100">
                                            <input type="text" id="ddVarietySearch" placeholder="Search..."
                                        ...
                                        <ul id="ddVarietyList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select variety">Select variety</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="sapphire" data-label="Sapphire">Sapphire</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="ruby" data-label="Ruby">Ruby</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddSubCategoryWrapper">
                                    <input type="hidden" name="idtbl_sub_categories" id="ddSubCategoryHidden"
                                        value="Unspecified">
                                    <button type="button" id="ddSubCategoryBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddSubCategoryPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddSubCategoryList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer bg-slate-100 text-slate-800 font-semibold"
                                                data-value="Unspecified" data-label="Unspecified">Unspecified</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="Natural" data-label="Natural">Natural</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="Synthetic" data-label="Synthetic">Synthetic</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <input type="text" name="length_mm" placeholder="L"
                                        class="form-control px-3 text-center !rounded-md">
                                    <span class="text-slate-600 font-medium text-[13px]">x</span>
                                    <input type="text" name="width_mm" placeholder="W"
                                        class="form-control px-3 text-center !rounded-md">
                                    <span class="text-slate-600 font-medium text-[13px]">x</span>
                                    <input type="text" name="height_mm" placeholder="H"
                                        class="form-control px-3 text-center !rounded-md">
                                </div>
                            </div>
                            ...
                            {{-- Weight --}}
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5">Weight <span
                                        class="text-rose-500">*</span></label>
                                <div class="flex gap-2">
                                    <input type="text" name="weight" placeholder='Weight e.g. "1.2"'
                                    ...
                                    <div class="relative w-[85px] searchable-dropdown" id="ddWeightUnitWrapper">
                                        ...
                                        <button type="button" id="ddWeightUnitBtn"
                                        ...
                                        <div id="ddWeightUnitPanel"
                                            class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                            <ul id="ddWeightUnitList"
                                                class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                                <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer bg-slate-100 text-slate-800 font-semibold"
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddColorWrapper">
                                    ...
                                    <button type="button" id="ddColorBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddColorPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <div class="p-2 border-b border-slate-100">
                                            <input type="text" id="ddColorSearch" placeholder="Search..."
                                                class="form-control !h-9 px-3">
                                        </div>
                                        <ul id="ddColorList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select color">Select color</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="blue" data-label="Blue">Blue</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="red" data-label="Red">Red</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddShapeWrapper">
                                    ...
                                    <button type="button" id="ddShapeBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddShapePanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <div class="p-2 border-b border-slate-100">
                                            <input type="text" id="ddShapeSearch" placeholder="Search..."
                                                class="form-control !h-9 px-3">
                                        </div>
                                        <ul id="ddShapeList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select shape">Select shape</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="oval" data-label="Oval">Oval</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddCuttingWrapper">
                                    ...
                                    <button type="button" id="ddCuttingBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddCuttingPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddCuttingList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select cutting type">Select cutting type</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="step" data-label="Step">Step</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="brilliant" data-label="Brilliant">Brilliant</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddColorGradeWrapper">
                                    ...
                                    <button type="button" id="ddColorGradeBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                        <span id="ddColorGradeLabel" class="truncate text-slate-400">Select color
                                            grade</span>
                                    </button>
                                    <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                                    ...
                                    <div id="ddColorGradePanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddColorGradeList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select color grade">Select color grade</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddClarityWrapper">
                                    ...
                                    <button type="button" id="ddClarityBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                        <span id="ddClarityLabel" class="truncate text-slate-400">Select clarity
                                            grade</span>
                                    </button>
                                    <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                                    ...
                                    <div id="ddClarityPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddClarityList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select clarity grade">Select clarity grade</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddCutGradeWrapper">
                                    ...
                                    <button type="button" id="ddCutGradeBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddCutGradePanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddCutGradeList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select cut grade">Select cut grade</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddOriginWrapper">
                                    ...
                                    <button type="button" id="ddOriginBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddOriginPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <div class="p-2 border-b border-slate-100">
                                            <input type="text" id="ddOriginSearch" placeholder="Search..."
                                                class="form-control !h-9 px-3">
                                        </div>
                                        <ul id="ddOriginList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select origin">Select origin</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddTreatmentWrapper">
                                    ...
                                    <button type="button" id="ddTreatmentBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddTreatmentPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <div class="p-2 border-b border-slate-100">
                                            <input type="text" id="ddTreatmentSearch" placeholder="Search..."
                                                class="form-control !h-9 px-3">
                                        </div>
                                        <ul id="ddTreatmentList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select treatment">Select treatment</li>
                                            @foreach($treatments as $treatment)
                                                <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                    data-value="{{ $treatment->idtbl_treatments }}"
                                                    data-label="{{ $treatment->treatment_name }}">
                                                    {{ $treatment->treatment_name }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddStorageWrapper">
                                    ...
                                    <button type="button" id="ddStorageBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                        <span id="ddStorageLabel" class="truncate text-slate-400">Select storage
                                            location</span>
                                    </button>
                                    <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                                    ...
                                    <div id="ddStoragePanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddStorageList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select storage location">Select storage location
                                            </li>
                                            @foreach($storageLocations as $location)
                                                <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                    data-value="{{ $location->idtbl_storage_locations }}"
                                                    data-label="{{ $location->location_name }}">{{ $location->location_name }}
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddTraysWrapper">
                                    ...
                                    <button type="button" id="ddTraysBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddTraysPanel"
                                        ...
                                        <ul id="ddTraysList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select trays/boxes#">Select trays/boxes#</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div
                            class="w-[180px] h-[90px] bg-white border border-slate-200 rounded-md flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 transition-colors shadow-sm mb-6">
                            <svg class="w-6 h-6 text-blue-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                                </path>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M12 9v2m0 0v2m0-2h2m-2 0H10"></path>
                            </svg>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5 mb-6">
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddSupplierWrapper">
                                    ...
                                    <button type="button" id="ddSupplierBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                        <span id="ddSupplierLabel" class="truncate text-slate-400">Select Supplier
                                            ref/name</span>
                                    </button>
                                    <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                                    ...
                                    <div id="ddSupplierPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddSupplierList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select Supplier ref/name">Select Supplier ref/name
                                            </li>
                                            @foreach($suppliers as $supplier)
                                                <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                    data-value="{{ $supplier->idtbl_suppliers }}"
                                                    data-label="{{ $supplier->supplier_name }}">{{ $supplier->supplier_name }}
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="date_of_purchase" placeholder="Date of purchase"
                            ...
                            <div>
                                ...
                                <div class="relative w-full searchable-dropdown" id="ddOwnershipWrapper">
                                    ...
                                    <button type="button" id="ddOwnershipBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                        <span id="ddOwnershipLabel" class="truncate text-slate-400">Select Ownership
                                            Type</span>
                                    </button>
                                    <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                                    ...
                                    <div id="ddOwnershipPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddOwnershipList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="" data-label="Select Ownership Type">Select Ownership Type</li>
                                            @foreach($ownershipTypes as $ownership)
                                                <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                    data-value="{{ $ownership->idtbl_ownership_type }}"
                                                    data-label="{{ $ownership->ownership_type_name }}">
                                                    {{ $ownership->ownership_type_name }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                            ...
                            <div class="col-span-1 md:col-span-3">
                                ...
                                <div id="partnerRowsContainer" class="flex flex-col gap-y-3">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 items-center">
                                        <div class="relative">
                                            <input type="hidden" name="my_company_id" value="1">
                                            <input type="text" value="My company" readonly
                                                class="form-control pl-3 pr-8 bg-slate-50 text-slate-600">
                                            <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                                <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                        ...
                                        <div class="relative">
                                            <input type="text" name="my_ownership_percentage" value="100"
                                                class="form-control pl-3 pr-8 bg-slate-50 text-slate-600" readonly>
                                            <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <div class="relative flex-1">
                                                <input type="text" name="my_profit_share_percentage" value="100"
                                                    class="form-control pl-3 pr-8 bg-slate-50 text-slate-600" readonly>
                                                <div
                                ...
                                <div class="flex justify-end mt-3">
                                    <button type="button" onclick="toggleAddPartnerModal(true)"
                                        class="px-3 py-1.5 bg-[#f1f5f9] text-[13px] text-slate-700 font-medium rounded-md hover:bg-slate-200">Add
                                        partner</button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="tab-pricing" class="tab-content hidden px-6 py-6 pb-20">
                        <div class="flex items-center gap-6 mb-6">
                            ...
                            <div class="w-64">
                                ...
                                <div class="relative searchable-dropdown" id="ddPricingUnitWrapper">
                                    ...
                                    <button type="button" id="ddPricingUnitBtn"
                                        class="form-control flex items-center pl-3 pr-8 text-left">
                                    ...
                                    <div id="ddPricingUnitPanel"
                                        class="hidden absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden">
                                        <ul id="ddPricingUnitList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer bg-slate-100 text-slate-800 font-semibold"
                                                data-value="Weight" data-label="Weight">Weight</li>
                                            <li class="dd-option flex items-center px-4 py-2.5 text-[14px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="Quantity" data-label="Quantity">Quantity</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mb-3">
                            <div>
                                ...
                                <input type="text" name="cost_per_unit" placeholder="Cost Per/unit"
                                    class="form-control px-3">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5 flex items-center gap-1">Total cost
                            ...
                            <div>
                                ...
                                <input type="text" name="my_cost_per_unit" placeholder="My cost Per/unit"
                                    class="form-control px-3">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5 flex items-center gap-1">My total cost
                                    <svg class="w-3.5 h-3.5 text-blue-500" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z">
                                ...
                                <input type="text" name="my_total_cost" placeholder="My total cost"
                                    class="form-control px-3">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mb-6">
                            <div>
                                ...
                                <input type="text" name="wholesale_per_unit" placeholder="Wholesale Cost Per/unit"
                                    class="form-control px-3">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5">Wholesale total</label>
                                <input type="text" name="wholesale_total" placeholder="Total Wholesale Cost"
                                    class="form-control px-3">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5">Retail / unit</label>
                                <input type="text" name="retail_per_unit" placeholder="Retail Cost Per/unit"
                                    class="form-control px-3">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5">Retail total</label>
                                <input type="text" name="retail_total" placeholder="Total Retail Cost"
                                    class="form-control px-3">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5">Matrix / unit</label>
                                <input type="text" name="matrix_per_unit" placeholder="Matrix Price Per/unit"
                                    class="form-control px-3">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5">Matrix total</label>
                                <input type="text" name="matrix_total" placeholder="Total Matrix Price"
                                    class="form-control px-3">
                            </div>
                        </div>
                        ...
                        <div
                            class="bg-[#f8fafc] px-4 py-3 border-b border-slate-100 flex font-semibold text-[13px] text-slate-800 rounded-b-md">
                    ...
                    <div id="tab-history" class="tab-content hidden px-6 py-6 pb-20">
                        <div class="flex justify-between items-center mb-6">
                            <div class="relative w-64 searchable-dropdown" id="ddFilterWrapper">
                                ...
                                <button type="button" id="ddFilterBtn"
                                    class="form-control flex items-center pl-3 pr-8 text-left bg-[#f8fafc] !border-slate-300">
                            ...
                            <div class="flex">
                                <input type="text" placeholder="Search"
                                    class="form-control !rounded-r-none !border-r-0 w-64 px-3">
                                <button type="button"
                                    class="bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded-r-md h-[42px] flex items-center justify-center">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <tbody class="text-slate-700 bg-white">
                                    @forelse($auditLogs ?? [] as $log)
                                        <tr class="border-b border-slate-50 hover:bg-slate-50">
                                            <td class="px-4 py-3">
                                                {{ \Carbon\Carbon::parse($log->insertdatetime)->format('Y-m-d') }}</td>
                                            <td class="px-4 py-3">
                                                {{ \Carbon\Carbon::parse($log->insertdatetime)->format('H:i:s') }}</td>
                                            ...
                                            <td class="px-4 py-3 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    <button type="button"
                                                        onclick='viewAuditLog({{ $log->entity_id }}, @json($log->action ?? ""), @json($log->user_name ?? "System"), @json($log->insertdatetime ?? ""))'
                                                    ...
                                                    <button type="button" onclick='editProduct({{ $log->entity_id }})'
                                                    ...
                                                    <button type="button" onclick='deleteAuditLog({{ $log->idtbl_audit_logs }})'
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="8" class="px-4 py-8 text-center text-slate-400">No activity history
                                                found.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                        ...
                        <div class="mt-4 flex justify-between items-center text-[13px] text-slate-600">
                            <div class="flex items-center gap-2">
                                ...
                                <div class="relative searchable-dropdown inline-block" id="ddShowEntriesWrapper">
                                    ...
                                    <button type="button" id="ddShowEntriesBtn"
                                        class="border border-slate-300 rounded-md px-2 py-1 focus:outline-none bg-white flex items-center gap-2">
                                    ...
                                    <div id="ddShowEntriesPanel"
                                        class="hidden absolute z-50 bottom-full mb-1 left-0 mt-2 bg-white border border-slate-200 rounded-md shadow-lg overflow-hidden min-w-[80px]">
                                        <ul id="ddShowEntriesList" class="py-1 max-h-48 overflow-y-auto custom-scrollbar">
                                            <li class="dd-option flex items-center px-4 py-2 text-[13px] cursor-pointer bg-slate-100 text-slate-800 font-semibold"
                                                data-value="10" data-label="10">10</li>
                                            <li class="dd-option flex items-center px-4 py-2 text-[13px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="25" data-label="25">25</li>
                                            <li class="dd-option flex items-center px-4 py-2 text-[13px] cursor-pointer hover:bg-slate-50 text-slate-600"
                                                data-value="50" data-label="50">50</li>
                                        </ul>
                                    </div>
                                </div>
                                entries
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <button type="button"
                                    class="px-3 py-1 bg-[#f1f5f9] border border-slate-200 rounded-md text-slate-700 font-medium">1</button>
            ...
            <div class="px-6 py-4 border-t border-slate-200 bg-white flex justify-end gap-3 flex-shrink-0 z-10">
                <button type="button" id="cancelGemstoneModalBtn"
                    class="px-6 py-2.5 text-[14px] font-medium text-rose-500 bg-white border border-rose-300 rounded-md hover:bg-rose-50 transition-colors">
                    Cancel
                </button>
                <button type="button"
                    class="px-6 py-2.5 text-[14px] font-medium text-slate-800 bg-[#f1f5f9] border border-slate-200 rounded-md hover:bg-slate-200 transition-colors">
                    Create and add another
                </button>
                <button type="submit" form="createGemstoneForm"
    ...
    <div id="auditLogViewModal"
        class="fixed inset-0 z-[10000] hidden items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-opacity p-4">
        <div
            class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col transform transition-all overflow-hidden border border-slate-200">
            {{-- Header --}}
            <div class="bg-slate-800 px-6 py-4 flex justify-between items-center text-white shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-[18px] font-bold tracking-wide uppercase flex items-center gap-2">GemExhibit
                            Enterprise <span class="text-blue-400 text-[14px] font-normal tracking-normal">- Product
                                Detailed View</span></h2>
                        <div class="text-[12px] text-slate-300 mt-0.5 flex items-center gap-3">
                            <span class="flex items-center"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor"
                                ...
                                </svg> SKU: <strong id="vLogSku" class="text-white ml-1">N/A</strong></span>
                            <span class="flex items-center"><svg class="w-3.5 h-3.5 text-emerald-400 mr-1" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg> Status: ACTIVE</span>
                            <span class="flex items-center"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10">
                                    </path>
                                </svg> Type: Diamond/Gemstone</span>
                        </div>
                    </div>
                </div>
                <button type="button" onclick="closeAuditLogViewModal()"
            ...
            <div class="p-6 overflow-y-auto custom-scrollbar bg-slate-50 flex-1 text-[13px] text-slate-700">
                ...
                <div class="bg-white rounded-lg border border-slate-200 p-5 mb-5 shadow-sm">
                    <h3
                        class="text-[14px] font-bold text-slate-800 uppercase tracking-wide border-b border-slate-100 pb-2 mb-4 flex items-center gap-2">
                        📦 1. Basic & Physical Details</h3>
                    <div class="grid grid-cols-2 gap-x-6 gap-y-3">
                        <div class="col-span-2 text-[14px]"><span
                                class="font-semibold text-slate-500 inline-block w-28">Product Title</span> : <span
                                id="vLogTitle" class="font-bold text-slate-900"></span></div>
                        <div class="col-span-2 text-[14px] mb-2"><span
                                class="font-semibold text-slate-500 inline-block w-28">Description</span> : <span
                                id="vLogDesc" class="text-slate-600"></span></div>

                        <div><span class="font-semibold text-slate-500 inline-block w-28">Category</span> : <span
                                id="vLogCategory"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Variety</span> : <span
                                id="vLogVariety"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Color / Grade</span> : <span
                                id="vLogColor"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Shape / Cut</span> : <span
                                id="vLogShape"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Treatment</span> : <span
                                id="vLogTreatment"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Origin</span> : <span
                                id="vLogOrigin"></span></div>
                        <div class="col-span-2"><span
                                class="font-semibold text-slate-500 inline-block w-28">Dimensions</span> : <span
                                id="vLogDims" class="font-medium text-slate-800"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Storage Loc.</span> : <span
                                id="vLogStorage"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Tray / Box</span> : <span
                                id="vLogTray"></span></div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg border border-slate-200 p-5 mb-5 shadow-sm">
                    <h3
                        class="text-[14px] font-bold text-slate-800 uppercase tracking-wide border-b border-slate-100 pb-2 mb-4 flex items-center gap-2">
                        💰 2. Pricing & Inventory Summary</h3>
                    <div class="grid grid-cols-2 gap-x-6 gap-y-3 mb-5">
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Selling Unit</span> : <span
                                id="vLogSellingUnit"></span></div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Quantity</span> : <span
                                id="vLogQuantity"></span> pcs</div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Total Weight</span> : <span
                                id="vLogWeight" class="font-medium text-slate-800"></span> ct</div>
                        <div><span class="font-semibold text-slate-500 inline-block w-28">Avg Wt/Pc</span> : <span
                                id="vLogAvgWt"></span> ct</div>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse rounded-md overflow-hidden ring-1 ring-slate-200">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr>
                                <td class="py-2.5 px-4 font-medium">Cost Price</td>
                                <td class="py-2.5 px-4 text-right" id="vLogCostPerUnit"></td>
                                <td class="py-2.5 px-4 text-right font-medium text-slate-800" id="vLogTotalCost"></td>
                            </tr>
                            <tr>
                                <td class="py-2.5 px-4 font-medium">My Cost (Partner)</td>
                                <td class="py-2.5 px-4 text-right" id="vLogMyCostPerUnit"></td>
                                <td class="py-2.5 px-4 text-right font-medium text-slate-800" id="vLogMyTotalCost"></td>
                            </tr>
                            <tr>
                                <td class="py-2.5 px-4 font-medium">Wholesale Price</td>
                                <td class="py-2.5 px-4 text-right" id="vLogWholesalePerUnit"></td>
                                <td class="py-2.5 px-4 text-right font-medium text-slate-800" id="vLogWholesaleTotal"></td>
                            </tr>
                            <tr>
                                <td class="py-2.5 px-4 font-semibold text-blue-600">Retail Price</td>
                                <td class="py-2.5 px-4 text-right text-blue-600 font-medium" id="vLogRetailPerUnit"></td>
                                <td class="py-2.5 px-4 text-right font-bold text-blue-700" id="vLogRetailTotal"></td>
                            </tr>
                            <tr>
                                <td class="py-2.5 px-4 font-medium">Matrix Price</td>
                                <td class="py-2.5 px-4 text-right" id="vLogMatrixPerUnit"></td>
                                <td class="py-2.5 px-4 text-right font-medium text-slate-800" id="vLogMatrixTotal"></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                    ...
                    <div class="bg-white rounded-lg border border-slate-200 p-5 shadow-sm">
                        <h3
                            class="text-[14px] font-bold text-slate-800 uppercase tracking-wide border-b border-slate-100 pb-2 mb-4 flex items-center gap-2">
                            🤝 4. Purchasing Info</h3>
                        <div class="space-y-3">
                            <div><span class="font-semibold text-slate-500 inline-block w-28">Supplier</span> : <span
                                    id="vLogSupplier"></span></div>
                            <div><span class="font-semibold text-slate-500 inline-block w-28">Contact</span> : <span
                                    id="vLogContact"></span></div>
                            <div><span class="font-semibold text-slate-500 inline-block w-28">Reference</span> : <span
                                    id="vLogSupplierRef"></span></div>
                            <div><span class="font-semibold text-slate-500 inline-block w-28">Date</span> : <span
                                    id="vLogPurchaseDate"></span></div>
                            <div><span class="font-semibold text-slate-500 inline-block w-28">Ownership</span> : <span
                                    id="vLogOwnership"></span></div>
                        </div>
                    </div>
                </div>
                ...
                {{-- Audit Context --}}
                <div
                    class="mt-6 text-center text-[12px] text-slate-400 border-t border-slate-200 pt-4 flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Log Type: <span id="vLogActionBadge"
                            class="font-bold text-slate-500 uppercase tracking-wide"></span> | Performed by <span
                            id="vLogUser" class="font-bold text-slate-500"></span> on <span id="vLogDate"
                            class="font-bold text-slate-500"></span></span>
                </div>
    ...
    <div id="auditLogEditModal"
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-[400px] transform transition-all p-6 text-left">
            ...
            <div class="mb-4">
                ...
                <textarea id="editLogNote"
                    class="w-full h-[100px] border border-slate-300 rounded-md px-3 py-2 text-[14px] focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"></textarea>
            </div>
            <div class="flex justify-end gap-3">
                <button type="button" onclick="closeAuditLogEditModal()"
                    class="px-5 py-2 text-[14px] font-medium text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors">Cancel</button>
                <button type="button" onclick="saveAuditLogEdit()"
                    class="px-5 py-2 text-[14px] font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">Save
                    Changes</button>
            </div>
        </div>
    </div>
    ...
    <div id="validationErrorModal"
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-[400px] transform transition-all p-6 text-center">
            ...
            <p class="text-[14px] text-slate-600 mb-4">Please fill in the following required fields before creating the
                product:</p>
            ...
            <button type="button" onclick="closeValidationModal()"
                class="w-full px-6 py-2.5 text-[14px] font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
            ...
            if (form) {
                form.addEventListener('submit', function (e) {
                    ...
                    if (errors.length > 0) {
                        ...
                        const originalBtnText = submitBtn ? submitBtn.innerHTML : 'Create';
                        if (submitBtn) {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Error!',
                                    text: data.message || 'An error occurred while saving the product.'
                                });
                            }
                        })
                        .catch(error => {
                ...
                fetch(`/Inventory/MyInventory/product-details/${id}`)
                    ...
                    .then(json => {
                        if (json.success && json.data.product) {
                            ...
                            if (p.inventory_type) {
                                const radio = document.querySelector(`input[name="inventory_type"][value="${p.inventory_type}"]`);
                                if (radio) {
                                    radio.checked = true;
                            ...
                            setVal('idtbl_suppliers', pu.idtbl_suppliers);
                            setVal('date_of_purchase', pu.date_of_purchase);
                            
                            // Partners
                            const partnerContainer = document.getElementById('partnersContainer');
                            if (partnerContainer) {
                                ...
                                pt.forEach(partner => {
                                    if (partner.name === 'My Company' || partner.name === 'My company') {
                                        setVal('my_ownership_percentage', partner.ownership);
                                        setVal('my_profit_share_percentage', partner.profit);
                                    } else {
                                        // Trigger Add Partner button if it exists, or manually create row
                                        const btn = document.getElementById('addPartnerBtn');
                                        if (btn) {
                                            btn.click();
                                            // The newly added row is the last one in the container
                                            const rows = partnerContainer.querySelectorAll('.partner-row');
                                            if (rows.length > 0) {
                                                const lastRow = rows[rows.length - 1];
                                                const idInput = lastRow.querySelector('input[name="partner_ids[]"]');
                                                const ownInput = lastRow.querySelector('input[name="ownership_percentages[]"]');
                                                const profInput = lastRow.querySelector('input[name="profit_percentages[]"]');
                                                
                                                if (idInput) idInput.value = partner.name; // In DB it might expect name or ID, but form uses name string or ID
                                                if (ownInput) ownInput.value = partner.ownership;
                                                if (profInput) profInput.value = partner.profit;
                                            }
                                        }
                                    }
                                });
