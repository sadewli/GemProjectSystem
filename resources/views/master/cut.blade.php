...
@section('title', 'Cuts - Master Data')
        ...
        <div class="card-body p-2">
            <div class="row">
                <div class="col-12 col-md-4 mb-3">
                    <form action="{{ url('Master/Cutinsertupdate') }}" method="post" autocomplete="off">
                        @csrf
                        ...
                        <div class="form-group mb-2">
                            ...
                            <input type="text" class="form-control form-control-sm" name="name" id="name" required placeholder="e.g. Brilliant">
                        </div>
                    ...
                    <div class="table-responsive pb-3">
                        <table class="table table-bordered table-striped table-sm nowrap w-100" id="dataTable">
                            ...
                            <tbody>
                                @forelse($cuts as $cut)
                                <tr>
                                    <td>{{ $cut->productType->name ?? '-' }}</td>
                                    <td>{{ $cut->name }}</td>
                                    <td class="text-right">
                                        <div class="btn-group btn-group-sm">
                                            <button type="button" class="btn btn-outline-primary btn-sm btnEdit" data-id="{{ $cut->idtbl_cuts }}" title="Edit"><i class="fas fa-pen"></i></button>
                                            <button type="button" class="btn btn-outline-danger btn-sm btnDelete" data-id="{{ $cut->idtbl_cuts }}" title="Delete"><i class="fas fa-trash-alt"></i></button>
                                        </div>
                                    </td>
                                </tr>
                                @empty
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btnEdit', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#recordID').val(response.idtbl_cuts);
                    $('#recordOption').val('2');
        ...
        $(document).on('click', '.btnDelete', function() {
            ...
            if (confirm('Are you sure you want to delete this record?')) {
                let form = $('<form method="POST" action="{{ url("Master/Cutdelete") }}"></form>');
