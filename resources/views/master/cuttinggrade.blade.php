...
@section('title', 'Cutting Grades - Master Data')
    ...
    <div class="container-fluid">
        <div class="page-header-content py-3 text-center text-lg-left">
            <h1 class="page-header-title font-weight-light mb-0">
                ...
                <span>Cutting Grades Master</span>
            </h1>
        </div>
    </div>
</div>
        ...
        <div class="card-body p-2">
            <div class="row">
                <div class="col-12 col-md-4 mb-3">
                    <form action="{{ url('Master/CuttingGradeinsertupdate') }}" method="post" autocomplete="off">
                        @csrf
                        ...
                        <div class="form-group mb-2">
                            <label class="small font-weight-bold text-dark">Cutting Grade Name*</label>
                            <input type="text" class="form-control form-control-sm" name="cuttinggradename" id="cuttinggradename" required placeholder="e.g. Excellent">
                        </div>
                    ...
                    <div class="table-responsive pb-3">
                        <table class="table table-bordered table-striped table-sm nowrap w-100" id="dataTable">
                            <thead class="thead-light">
                                <tr>
                                    <th>PRODUCT TYPE</th>
                                    <th>CUTTING GRADE NAME</th>
                                    <th class="text-right">ACTIONS</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($cuttinggrades as $grade)
                                <tr>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btnEdit', function() {
            ...
            $.ajax({
                url: '{{ url("Master/CuttingGradeedit") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    recordID: recordID
                ...
                success: function(response) {
                    $('#cuttinggradename').val(response.cuttinggradename);
                    $('#idtbl_product_types').val(response.idtbl_product_types);
                    $('#recordID').val(response.idtbl_cuttinggrade);
                    $('#recordOption').val('2');
        ...
        $(document).on('click', '.btnDelete', function() {
            ...
            if (confirm('Are you sure you want to delete this record?')) {
                let form = $('<form method="POST" action="{{ url("Master/CuttingGradedelete") }}"></form>');
