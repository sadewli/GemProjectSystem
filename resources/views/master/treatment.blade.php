...
@section('title', 'Treatments - Master Data')
        ...
        <div class="card-body p-2">
            <div class="row">
                <div class="col-12 col-md-4 mb-3">
                    <form action="{{ url('Master/Treatmentinsertupdate') }}" method="post" autocomplete="off">
                        @csrf
                        ...
                        <div class="form-group mb-2">
                            <label class="small font-weight-bold text-dark">Treatment Name*</label>
                            <input type="text" class="form-control form-control-sm" name="treatment_name" id="treatment_name" required placeholder="e.g. Heated">
                        </div>
                    ...
                    <div class="table-responsive pb-3">
                        <table class="table table-bordered table-striped table-sm nowrap w-100" id="dataTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $treatment->productType->name ?? '-' }}</td>
                                    <td>{{ $treatment->treatment_name }}</td>
                                    <td class="text-right">
                                        <div class="btn-group btn-group-sm">
                                            <button type="button" class="btn btn-outline-primary btn-sm btnEdit" data-id="{{ $treatment->idtbl_treatments }}" title="Edit"><i class="fas fa-pen"></i></button>
                                            <button type="button" class="btn btn-outline-danger btn-sm btnDelete" data-id="{{ $treatment->idtbl_treatments }}" title="Delete"><i class="fas fa-trash-alt"></i></button>
                                        </div>
                                    </td>
                                </tr>
                                @empty
        ...
        $(document).on('click', '.btnEdit', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#recordID').val(response.idtbl_treatments);
                    $('#recordOption').val('2');
