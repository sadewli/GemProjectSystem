   ...
   static function data_output ( $columns, $data, $isJoin = false )
{
    ...
    for ($i = 0, $ien = count($data); $i < $ien; $i++) {
        ...
        for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
            $column = $columns[$j];
            $fieldName = isset($column['field']) ? $column['field'] : $column['db'];

            // Use field names as JSON keys
            if (isset($column['formatter'])) {
                $row[$fieldName] = $column['formatter']($data[$i][$fieldName], $data[$i]);
            } else {
                if (isset($data[$i][$fieldName])) {
                    $row[$fieldName] = $data[$i][$fieldName];
                } else {
                    $row[$fieldName] = null;
                }
            }
